...
public class Day21 extends Day {
  ...
  record Op(String operation, OpType opType) {
    enum OpType {
    ...
    public void scramble(char[] password) {
      opType.scramble(operation, password);
    }

    public void unscramble(char[] password) {
      opType.unscramble(operation, password);
    }

    static Op fromLine(String operation) {
      return new Op(operation, Arrays.stream(OpType.values()).filter(o -> operation.startsWith(o.prefix)).findFirst().orElseThrow());
    }
  }
  ...
  String part1() {
    ...
    input.stream().map(Op::fromLine).forEach(op -> op.scramble(password));
  ...
  String part2() {
    ...
    input.reversed().stream().map(Op::fromLine).forEach(op -> op.unscramble(password));
    return new String(password);
  }
